...
![](https://raw.githubusercontent.com/jiannei/images/main/images/202502252052542.png)

## 自增值保存在哪里？

使用 `insert into test_pk values(null, 1, 1)` 插入一行数据，再执行 `show create table` 命令来看一下表的结构定义：

![](https://raw.githubusercontent.com/jiannei/images/main/images/202502252052118.png)

上述表的结构定义存放在后缀名为 `.frm` 的本地文件中，在 MySQL 安装目录下的 data 文件夹下可以找到这个 `.frm` 文件：

![](https://raw.githubusercontent.com/jiannei/images/main/images/202502252052187.png)
...
![](https://raw.githubusercontent.com/jiannei/images/main/images/202502252053396.png)

但如果马上重启 MySQL 实例，重启后这个表的 AUTO_INCREMENT 就会变成 1。﻿ 也就是说，MySQL 重启可能会修改一个表的 AUTO_INCREMENT 的值。

![](https://raw.githubusercontent.com/jiannei/images/main/images/202502252053462.png)

![](https://raw.githubusercontent.com/jiannei/images/main/images/202502252053703.png)
...
![](https://raw.githubusercontent.com/jiannei/images/main/images/202502252054056.png)

这时我再执行一条插入 `(null,1,1)` 的命令，很显然会报错 `Duplicate entry`，因为我们设置了一个唯一索引字段 `a`：

![](https://raw.githubusercontent.com/jiannei/images/main/images/202502252054081.png)
...
![](https://raw.githubusercontent.com/jiannei/images/main/images/202502252055186.png)

我们先插入一行数据 `(null, 2, 2)`，也就是 (3, 2, 2) 嘛，并且 AUTO_INCREMENT 变为 4：

![](https://raw.githubusercontent.com/jiannei/images/main/images/202502252055763.png)

再去执行这样一段 SQL：

![](https://raw.githubusercontent.com/jiannei/images/main/images/202502252055614.png)

虽然我们插入了一条 (null, 3, 3) 记录，但是使用 rollback 进行回滚了，所以数据库中是没有这条记录的：

![](https://raw.githubusercontent.com/jiannei/images/main/images/202502252056575.png)

在这种事务回滚的情况下，自增值并没有同样发生回滚！如下图所示，自增值仍然固执地从 4 增加到了 5：

![](https://raw.githubusercontent.com/jiannei/images/main/images/202502252056124.png)

所以这时候我们再去插入一条数据（null, 3, 3）的时候，主键 id 就会被自动赋为 `5` 了：

![](https://raw.githubusercontent.com/jiannei/images/main/images/202502252056843.png)
...
![](https://raw.githubusercontent.com/jiannei/images/main/images/202502252057234.png)
...
![](https://raw.githubusercontent.com/jiannei/images/main/images/202502252058064.png)

我们创建一个和当前表 `test_pk` 有相同结构定义的表 `test_pk2`：

![](https://raw.githubusercontent.com/jiannei/images/main/images/202502252058591.png)

然后使用 `insert...select` 往 `teset_pk2` 表中批量插入数据：

![](https://raw.githubusercontent.com/jiannei/images/main/images/202502252059406.png)

可以看到，成功导入了数据。

再来看下 `test_pk2` 的自增值是多少：

![](https://raw.githubusercontent.com/jiannei/images/main/images/202502252059291.png)
...
![](https://raw.githubusercontent.com/jiannei/images/main/images/202502252059099.png)
